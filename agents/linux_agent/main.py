...
from agents.linux_agent.config import (
    LinuxAgentSettings,
    load_agent_settings,
)

from agents.linux_agent.executor import (
    execute_script,
)

from agents.linux_agent.platform_client import (
    append_job_log,
    claim_next_job,
    complete_job,
    download_job_script,
...
logger = logging.getLogger(
    "automation_platform_linux_agent"
)


def configure_logging() -> None:
    """
    Configure console logging for the Linux Agent.
    """
...
def run_agent(
    ...
    """
    Run the Linux Agent heartbeat and job-processing loop.
    ...
    logger.info(
        "Linux Agent loop stopped gracefully."
    )


def main() -> int:
    """
    Linux Agent application entry point.
    """
    ...
    def request_shutdown(
        ...
        logger.info(
            "Linux Agent shutdown requested. "
            "Signal: %s",
            signum,
        )
    ...
    try:
        ...
        logger.info(
            "Linux Agent stopped by the user."
        )
